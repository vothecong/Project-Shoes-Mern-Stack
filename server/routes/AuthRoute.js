...
router.post("/check-email-exist", async (req, res, next) => {
    const KEY_CONFIRM_EMAIL =
        "0v9StzSEJL0XfcGu76M7ATKZoAH523TPakrSoVKqJnMMgoi4z3";
    const result = await fetch(
        `https://app.verify-email.org/api/v1/${KEY_CONFIRM_EMAIL}/verify/${req.body.email}`
    ).then((res) => res.json());
...
router
    .route("/admin/get-account")
    .get(passport.authenticate("jwt", { session: false }), auth.getAccountAdmin);
...
router
    ...
    .post(passport.authenticate("jwt", { session: false }), auth.updateAvatar);

router
    .route("/user/get-order-by-cusomer")
    .get(
        passport.authenticate("jwt", { session: false }),
